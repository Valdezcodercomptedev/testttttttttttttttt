...
class CompanyServices
{
    ...
    public function update(array|Company $company): bool
    {
        ...
        if (empty($existedCompany->getId())) {
            $sql = "INSERT INTO company(name, director_name, address, tel1, tel2, email, created, modified) VALUES(?,?,?,?,?,?,?,?)";
        } else {
            $sql = "UPDATE company SET name = ?, director_name = ?, address = ?, tel1 = ?, tel2 = ?, email = ?, modified = ? WHERE id = ?";
        }
        ...
        try {
            ...
            if (empty($existedCompany->getId())) {
                ...
                $query->bindValue(7, $company->getCreated(), \PDO::PARAM_STR);
                $query->bindValue(8, $company->getModified(), \PDO::PARAM_STR);
            } else {
                $query->bindValue(1, $company->getName(), \PDO::PARAM_STR);
                $query->bindValue(2, $company->getDirectorName(), \PDO::PARAM_STR);
                $query->bindValue(3, $company->getAddress(), \PDO::PARAM_STR);
                ...
                $query->bindValue(5, $company->getTel2(), \PDO::PARAM_STR);
                $query->bindValue(6, $company->getEmail(), \PDO::PARAM_STR);
                $query->bindValue(7, $company->getModified(), \PDO::PARAM_STR);
                $query->bindValue(8, $existedCompany->getId(), \PDO::PARAM_INT);
            }
